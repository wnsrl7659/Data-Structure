...
\date   6/17/2019
\brief
  This is the header file for Suduku.
*/
/******************************************************************************/
//---------------------------------------------------------------------------
#ifndef SUDOKUH
...
class Sudoku
{
  ...
  private:
	  ...
	  SudokuStats stats; //saving stat value
	  SUDOKU_CALLBACK cb; // saving call back function
	  SymbolType type; // saving type number or letter
	  char *board; // saving board
	  int *check; // saving const value check. is this value already in map? 
	  int totalsize; // for good life

	  MessageType Solving(int index, char start); // recursive  fucntion
	  int Checking(int index, int type); // chcking conflick, row-col-box conflict
};
